...
window.addEventListener('DOMContentLoaded', function() {
  ...
  var createScene = async function() {
    ...
    // matGrass.texture
    var grass = "./assets/untitled-q.png";
    ...
    // matGrass.texture
    var grass2 = "./assets/sphere-q.jpg";
    ...
    // Initialize wind particle system with optimized count
    windParticles = new WindParticleSystem(scene, 15000);
    console.log("Using 15,000 optimized wind particles");
  ...
  let mQ = [];
  let gY = 0;
  let windParticles; // Global variable for particle system
  ...
  async function begin(){
    ...
    engine.runRenderLoop(function() {
      ...
      if (typeof windParticles !== 'undefined') {
        windParticles.updateParticles(engine.getDeltaTime() / 1000);
      } else {
        console.log("Wind particles not initialized");
      }
    ...
    if (squeeze){
      ...
      // Optimized bobbing - preserving original effect
      for(var i=0;i<mQ.length; i++){
        var tt = mQ[i];
        // Original pattern: cos(x+gY) + sin(z+gY) - just optimized variable usage
        var xPhase = tt.position.x + gY;
        var zPhase = tt.position.z + gY;
        tt.position.y = Math.cos(xPhase) + Math.sin(zPhase);
      }
      // console.log(scene)
      scene.render();
    });
