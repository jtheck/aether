...
`;function u(e){return e.replace(/[^A-Za-z0-9_]/g,"_")}function f(e,o){return typeof e=="number"?`${e}`:`${o}`}var v={className:"AmbientOcclusionBlock",stage:"fragment",emit(e,o,l,t,a){var c;let n=(c=e.inputs.get("source"))==null?void 0:c.source;if(!n)throw new Error(`NodeMaterial: AmbientOcclusionBlock "${e.name}" requires an ImageSourceBlock source`);let p=a.graph.blocks.get(n.blockId),r=u(p?.name||`ao${e.id}`);t.textures.find(d=>d.name===r)||t.textures.push({name:r,kind:"texture2d",texture:null}),t.fragment.helpers.set(i,m);let s=a.cast(a.resolve(e,"screenSize",l,t),"vec2f").expr;return{expr:`nme_computeAo(nodeTex_${r}, nodeSamp_${r}, _NME_FRAG_COORD_, ${s}, ${f(e.serialized.radius,1e-4)}, ${f(e.serialized.area,.0075)}, ${f(e.serialized.fallOff,1e-6)})`,type:"f32"}}};export{v as emitter};
