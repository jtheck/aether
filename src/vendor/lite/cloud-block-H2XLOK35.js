function i(c,v,o,e,n){return c.inputs.get(v)?.source?n.resolve(c,v,o,e):null}function u(c,v,o,e,n){let t=i(c,v,o,e,n);return t?n.cast(t,"f32").expr:null}var _={className:"CloudBlock",emit(c,v,o,e,n){let t=n.resolve(c,"seed",o,e);if(t.type!=="vec2f"&&t.type!=="vec3f")throw new Error(`NodeMaterial: CloudBlock requires vec2 or vec3 seed; got ${t.type}`);e[o].helpers.set("nme_cloudNoise",`fn nme_cloudRandom(p: f32) -> f32 {
...
return value;
}`);let m=n.temp(e,"cloudSeed");e[o].body.push(`var ${m}: ${t.type==="vec2f"?"vec2<f32>":"vec3<f32>"} = ${t.expr};`);let d=u(c,"offsetX",o,e,n),p=u(c,"offsetY",o,e,n),l=u(c,"offsetZ",o,e,n);d&&e[o].body.push(`${m}.x += 0.1 * ${d};`),p&&e[o].body.push(`${m}.y += 0.1 * ${p};`),l&&t.type==="vec3f"&&e[o].body.push(`${m}.z += 0.1 * ${l};`);let r=i(c,"chaos",o,e,n),s=r?n.cast(r,t.type).expr:t.type==="vec2f"?"vec2<f32>(0.0, 0.0)":"vec3<f32>(0.0, 0.0, 0.0)";return{expr:t.type==="vec2f"?`nme_cloudFbm2_${f}(${m}, ${s})`:`nme_cloudFbm3_${f}(${m}, ${s})`,type:"f32"}}};export{_ as emitter};
