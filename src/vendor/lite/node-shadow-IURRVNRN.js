import{a as f}from"./chunk-VQHJJFNH.js";import{c as o}from"./chunk-VCOYERVR.js";var c=`array<f32, ${f}>`,$=`${c}(${new Array(f).fill("1.0").join(", ")})`;function _(d,w,h){let l=[],n=[],u=[];for(let t of d){let e=`_${t.lightIndex}`;h.some(p=>p._name===`vPosFromLight${e}`)||h.push({_name:`vPosFromLight${e}`,_type:"vec4<f32>"}),h.some(p=>p._name===`vDepthMetric${e}`)||h.push({_name:`vDepthMetric${e}`,_type:"f32"})}let m=["let _shadowWp4 = meshU.world * vec4<f32>(in.position, 1.0);"],s=[`var _sf = ${$};`],r=w;for(let t of d){let e=`_${t.lightIndex}`,p=t.lightIndex,a=r++,i=r++,v=r++,x=t.shadowType;l.push({_lightIndex:p,_texBinding:a,_sampBinding:i,_uboBinding:v,_shadowType:x}),n.push(`struct shadowInfo${e}Uniforms { lightMatrix: mat4x4<f32>, depthValues: vec4<f32>, shadowsInfo: vec4<f32> };`,`@group(1) @binding(${v}) var<uniform> shadowInfo${e}: shadowInfo${e}Uniforms;`),t.shadowType==="pcf"?(n.push(`@group(1) @binding(${a}) var shadowTex${e}: texture_depth_2d;`,`@group(1) @binding(${i}) var shadowComp${e}: sampler_comparison;`,`fn computeShadowPCF${e}(posFromLight: vec4<f32>, depthMetric: f32, darkness: f32, mapSz: f32, invMapSz: f32) -> f32 {
...
}`),s.push(`_sf[${t.lightIndex}] = computeShadowPCF${e}(input.vPosFromLight${e}, input.vDepthMetric${e}, shadowInfo${e}.shadowsInfo.x, shadowInfo${e}.shadowsInfo.y, shadowInfo${e}.shadowsInfo.z);`),u.push({binding:a,visibility:o.FRAGMENT,texture:{sampleType:"depth",viewDimension:"2d"}},{binding:i,visibility:o.FRAGMENT,sampler:{type:"comparison"}})):(n.push(`@group(1) @binding(${a}) var shadowTex${e}: texture_2d<f32>;`,`@group(1) @binding(${i}) var shadowSamp${e}: sampler;`,`fn computeFallOff${e}(value: f32, clipSpace: vec2<f32>, frustumEdgeFalloff: f32) -> f32 {
...
}`),s.push(`_sf[${t.lightIndex}] = computeShadowESM${e}(input.vPosFromLight${e}, input.vDepthMetric${e}, shadowInfo${e}.shadowsInfo.x, shadowInfo${e}.shadowsInfo.z, shadowInfo${e}.shadowsInfo.w);`),u.push({binding:a,visibility:o.FRAGMENT,texture:{sampleType:"float",viewDimension:"2d"}},{binding:i,visibility:o.FRAGMENT,sampler:{type:"filtering"}})),m.push(`out.vPosFromLight${e} = shadowInfo${e}.lightMatrix * _shadowWp4;`,`out.vDepthMetric${e} = (out.vPosFromLight${e}.z + shadowInfo${e}.depthValues.x) / shadowInfo${e}.depthValues.y;`),u.push({binding:v,visibility:o.VERTEX|o.FRAGMENT,buffer:{type:"uniform",minBindingSize:96}})}return s.push(`for (var _i = 0u; _i < ${f}u; _i++) { _sf[_i] = mix(1.0, _sf[_i], meshU.receivesShadow.x); }`),s.push("return _sf;"),{_bindings:l,_wgslDecls:n.join(`
`),_fragmentHelper:`fn nme_computeShadowFactors(input: VertexOut) -> ${c} {
    ${s.join(`
    `)}
}`,_vertexInject:m.join(`
    `),_bglEntries:u,_bindingCount:d.length*3}}export{_ as emitShadow};
